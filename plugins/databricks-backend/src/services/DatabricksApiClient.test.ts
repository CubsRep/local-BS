...
import { DatabricksClient } from './DatabricksApiClient';
...
describe('DatabricksClient', () => {
  ...
  describe('getAccessToken', () => {
    it('should fetch and cache a new access token', async () => {
      ...
      mockedFetch.mockResolvedValueOnce(
        makeResponse({ ok: true, status: 200, json: { access_token: mockAccessToken } }),
      );
    ...
    it('should return cached token if not expired', async () => {
      ...
      // @ts-ignore
      client.tokenCache = { token: mockAccessToken, expiresAtMs: Date.now() + 10_000 };
    ...
    it('should throw an error if token API call fails', async () => {
      ...
      await expect(client.getAccessToken()).rejects.toThrow('Databricks token error: 500');
      expect(mockedFetch).toHaveBeenCalledTimes(1);
    });

    it('should throw an error if token response is invalid JSON', async () => {
      mockedFetch.mockResolvedValueOnce(
        makeResponse({ ok: true, status: 200, jsonThrows: true, text: '<html>Invalid JSON</html>' }),
      );

      await expect(client.getAccessToken()).rejects.toThrow('Failed to parse Databricks token response');
      expect(mockedFetch).toHaveBeenCalledTimes(1);
    });
  ...
  describe('listWorkspaceNamesCached', () => {
    it('should fetch and cache new workspace names', async () => {
      ...
      mockedFetch
        .mockResolvedValueOnce(makeResponse({ ok: true, status: 200, json: { access_token: mockAccessToken } }))
        .mockResolvedValueOnce(makeResponse({ ok: true, status: 200, json: mockWorkspaces }));
      ...
      // @ts-ignore
      expect(client.workspaceNamesCache.names).toEqual(['workspace-a', 'workspace-b']);
      // @ts-ignore
      expect(client.workspaceNamesCache.expiresAtMs).toBeGreaterThan(Date.now());
    });

    it('should return cached workspace names if not expired', async () => {
      const mockCachedNames = ['cached-ws-a', 'cached-ws-b'];
      // @ts-ignore
      client.workspaceNamesCache = { names: mockCachedNames, expiresAtMs: Date.now() + 10_000 };
    ...
    it('should throw an error if workspace API call fails', async () => {
      mockedFetch
        .mockResolvedValueOnce(makeResponse({ ok: true, status: 200, json: { access_token: 'mock-token' } }))
        .mockResolvedValueOnce(makeResponse({ ok: false, status: 500, text: 'Internal Server Error' }));

      await expect(client.listWorkspaceNamesCached()).rejects.toThrow('Databricks workspace error: 500');
      expect(mockedFetch).toHaveBeenCalledTimes(2);
    });
    ...
    it('should throw an error if workspace response is not an array', async () => {
      mockedFetch
        .mockResolvedValueOnce(makeResponse({ ok: true, status: 200, json: { access_token: 'mock-token' } }))
        .mockResolvedValueOnce(makeResponse({ ok: true, status: 200, json: { data: 'not-an-array' } }));
  ...
  describe('isWorkspaceNameAvailable', () => {
    ...
    it('should return false if the workspace name is not available (case-insensitive)', async () => {
      primeTokenAndWorkspaceMocks();
      const available = await client.isWorkspaceNameAvailable('Existing-Workspace');
      expect(available).toBe(false);
    });

    it('should throw an error for an empty workspace name', async () => {
      await expect(client.isWorkspaceNameAvailable('')).rejects.toThrow('Workspace name cannot be empty');
      expect(mockedFetch).not.toHaveBeenCalled();
    });
    ...
    it('should throw an error for a workspace name with invalid characters', async () => {
      await expect(client.isWorkspaceNameAvailable('invalid!name')).rejects.toThrow(
